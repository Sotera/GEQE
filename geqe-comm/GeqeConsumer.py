...
Usage:
cp geqeconf.py.template  geqeconf.py (edit for your system / settings)
python GeqeConsumer.py  (requires a geqe-dataserver instance to be running)
...
class JobRunner(Thread):
    ...
    def executeJob(self,job):
        ...
        if conf.MOCK_DATA_PATH is not None:
            return self.executeMockJob(job)
...
if __name__ == '__main__':
    ...
    # insert mock dataset for front end testing
    if conf.MOCK_DATA_PATH is not None:
        for shape in [1,2]:
